...
public:
    ...
    cv::Mat getEyeImage(const cv::Mat img, const LandmarkDetector::CLNF &model, int pos, int step, bool clacElse, float &quality);
    void getCLNFBox(const LandmarkDetector::CLNF &model, int pos, int step, double &X, double &Y, double &W, double &H);
    void printSmallImage(cv::Mat img,const LandmarkDetector::CLNF &model, QPainter &painterR, QPainter &painterL, bool print, std::string titel, bool drawLandmarks,
